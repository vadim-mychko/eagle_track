...
void Tracker::callbackExchange(const sensor_msgs::ImageConstPtr& img_msg, const sensor_msgs::ImageConstPtr& depth_msg, CameraContext& self, CameraContext& other) {
  ...
  auto bridge = cv_bridge::toCvShare(depth_msg);
  ...
  for (int w = 0; w <= bbox.width; ++w) {
    for (int h = 0; h <= bbox.height; ++h) {
      ...
      double depth_num = depth.at<uint16_t>({corner.x, corner.y}) * 1e-3;
      near.point.z = depth_num;
...
bool Tracker::processExchange(CameraContext& cc) {
  if (cc.success || !cc.got_exchange || cc.buffer.empty()) {
    return false;
  }
